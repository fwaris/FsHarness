...
FsHarness enables MSBuild hard links for Codex workers and deterministic evaluators. On a single filesystem, large copy-local native dependencies share physical storage with the package cache while retaining normal worktree paths. Terminal experiment worktrees are removed after durable results and lineage are recorded; active, awaiting-review, and recovery worktrees are retained.
...
- Before candidate capture or evaluator execution, FsHarness fingerprints the complete editable Git tree and compares it with prior candidate refs in the run. An exact duplicate receives one regeneration prompt containing the fingerprint and matching experiment ID; a repeated duplicate is recorded but consumes neither an experiment slot nor a non-improvement slot.
